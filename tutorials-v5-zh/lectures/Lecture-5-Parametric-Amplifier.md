...
### 二阶相干函数：Cauchy-Schwarz 不等式

Walls 和 Milburn 第 78 页：经典态满足

$[g_{12}^{(2)}]^2 \leq g_{1}^{(2)}g_{2}^{(2)}$

（Cauchy-Schwarz 不等式的一种形式）
...
显然，参量放大器产生的双模压缩态不满足该不等式，因此不是经典态。


Cauchy-Schwarz 不等式
-------------------------

Walls 和 Milburn 第 78 页：对称模下
...
量子-经典判据 $\langle:f^\dagger f:\rangle$
---------------------------------------------------------

用 $\langle:f^\dagger f:\rangle$ 也可再次说明参量放大器输出是非经典的。若取
...
纠缠：对数负性（logarithmic negativity）
------------------------------------


#### Wigner 协方差矩阵：

计算对数负性前，需先构造 Wigner 协方差矩阵：
