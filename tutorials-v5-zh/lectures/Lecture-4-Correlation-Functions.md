...
对于相干态，$|g^{(2)}(\tau)| = 1$；对于热态，$g^{(2)}(\tau) = 2$（光子聚束，更倾向成团出现）。 

下面代码计算并绘制 $g^{(2)}(\tau)$ 随 $\tau$ 的变化。
...
定义：对一个取值满足 $|Q(t)| \leq 1$ 的可观测量 $Q(t)$，若同时假设

 * 宏观实在性（macroscopic realism）
 * 非侵入测量（noninvasive measurements）
...
若测量初时刻系统已在稳态，可取 $\tau = t_1 = t_2$，此时 Leggett-Garg 不等式写为
...
<!-- #region -->
### 示例：两个耦合谐振器（光力学系统）的 Leggett-Garg 不等式
...
### 计算关联函数 $\langle Q(t_1+t_2)Q(t_1)\rangle$

使用回归定理以及 QuTiP 的 `correlation` 函数。
