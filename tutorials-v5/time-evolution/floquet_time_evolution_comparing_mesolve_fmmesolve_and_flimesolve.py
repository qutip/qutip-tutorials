...
"""
import numpy as np
from qutip import (Qobj,about, basis, flimesolve,fmmesolve, fsesolve,mesolve,
                   plot_expectation_values, sigmax, sigmaz,destroy)
...
ax[3,0].plot(tlist/T,abs(mestates[:,0,0]-fmmestates[:,0,0])*100, color = 'darkolivegreen',alpha=0.5)
ax[3,0].plot(tlist/T,abs(mestates[:,0,0]-flimestates[:,0,0])*100, color = 'rebeccapurple',linestyle = '--')
ax[3,0].legend(['FMME % dev from ME element [0,0]','FLiME % dev from ME element [0,0]'])
ax[3,0].set_ylabel('Percent Deviation') 
ax[3,0].set_xlabel('time (in periods $\\tau$ of the Hamiltonian)')

ax[3,1].plot(tlist/T,abs(mestates[:,0,1]-fmmestates[:,0,1])*100, color = 'darkolivegreen',alpha=0.5)
ax[3,1].plot(tlist/T,abs(mestates[:,0,1]-flimestates[:,0,1])*100, color = 'rebeccapurple',linestyle = '--')
ax[3,1].legend(['FMME % dev from ME element [0,1]','FLiME % dev from ME element [0,1]'])
ax[3,1].set_xlabel('time (in periods $\\tau$ of the Hamiltonian)')

ax[3,2].plot(tlist/T,abs(mestates[:,1,0]-fmmestates[:,1,0])*100, color = 'darkolivegreen',alpha=0.5)
ax[3,2].plot(tlist/T,abs(mestates[:,1,0]-flimestates[:,1,0])*100, color = 'rebeccapurple',linestyle = '--')
ax[3,2].legend(['FMME % dev from ME element [1,0]','FLiME % dev from ME element [1,0]'])
ax[3,2].set_xlabel('time (in periods $\\tau$ of the Hamiltonian)')


ax[3,3].plot(tlist/T,abs(mestates[:,1,1]-fmmestates[:,1,1])*100, color = 'darkolivegreen',alpha=0.5)
ax[3,3].plot(tlist/T,abs(mestates[:,1,1]-flimestates[:,1,1])*100, color = 'rebeccapurple',linestyle = '--')
ax[3,3].legend(['FMME % dev from ME element [1,1]','FLiME % dev from ME element [1,1]'])
ax[3,3].set_xlabel('time (in periods $\\tau$ of the Hamiltonian)')
fig.suptitle("Evolution of driven two level system - absolute percent deviation of fmmesolve and flimesolve from mesolve" )